...
for j = 1:length(SNR)
    ...
    for i =1:num_samples
        ...
        if mod(i, num_samples2)==1
            ...
            Psi1_values = abs(log((exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(5*pi/4+Phi_h)+1i*sin(5*pi/4+Phi_h))))).^2)+exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(-1*pi/4+Phi_h)+1i*sin(-1*pi/4+Phi_h))))).^2))./(exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(1*pi/4+Phi_h)+1i*sin(1*pi/4+Phi_h))))).^2)+exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(3*pi/4+Phi_h)+1i*sin(3*pi/4+Phi_h))))).^2))));
            Psi2_values = abs(log((exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(3*pi/4+Phi_h)+1i*sin(3*pi/4+Phi_h))))).^2)+exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(5*pi/4+Phi_h)+1i*sin(5*pi/4+Phi_h))))).^2))./(exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(1*pi/4+Phi_h)+1i*sin(1*pi/4+Phi_h))))).^2)+exp(-(abs(Y_h-(sqrt(2)*H_h*a.*(cos(-1*pi/4+Phi_h)+1i*sin(-1*pi/4+Phi_h))))).^2))));
        end
        y1 = Y1(i);
