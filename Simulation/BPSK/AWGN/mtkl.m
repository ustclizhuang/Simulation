...
for j = 1:length(SNR)
    ...
    angle = [+1, -1];
    points(:,1) = angle(idx).*sqrt(P)+randn(1, num_samples);
    points(:,2) = sqrt(P)+randn(1, num_samples);
    points(:,3) = -sqrt(P)+randn(1, num_samples);
    ...
    for i = 1:num_samples
        y1 = Y1(i);
        q_0=1/(sqrt(2*pi))*exp(-(y1-sqrt(P))^2/2);
        q_1=1/(sqrt(2*pi))*exp(-(y1+sqrt(P))^2/2);
        if q_0 < q_1
            t = abs(log(q_0/q_1));
        ...
        y2 = Y2(i);
        q_0=1/(sqrt(2*pi))*exp(-(y2-sqrt(P))^2/2);
        q_1=1/(sqrt(2*pi))*exp(-(y2+sqrt(P))^2/2);
        if q_0 > q_1
            t = abs(log(q_0/q_1));
            val = sum(Psi_values <= t) / length(Psi_values);
            sum12 = sum12 + val;
...
file_path = fullfile(pwd, sprintf('data_AWGN_BPSK.mat'));
save(file_path, 'data');